...
function FavoriteCommunityPosts() {
    const { userProfile, logout } = useAuth();
    const [favoriteCommunityPosts, setFavoriteCommunityPosts] = useState(null);

    useEffect(() => {
        const fetch = async () => {
            const token = localStorage.getItem('accessToken');
            const response = await axios.get(`http://localhost:8080/authapi/favoritecommunityposts?userId=${userProfile.id}`, {
                headers: { 'Authorization': `Bearer ${token}` },
            });
            if (response.status === 200) {
                console.log(response.data)
                setFavoriteCommunityPosts(response.data.favoriteCommunityPosts);
            } else {
                logout();
            }
        };
    ...
    return (
        <table className={styles.itemContainer}>
            ...
            <tbody>
                {generateList(favoriteCommunityPosts)}
            </tbody>
        </table>
    )
}
export default FavoriteCommunityPosts;