...
function SearchBox() {
    ...
    function generateBoard(Lists) {
        if(Lists && Lists.length > 0) {
            const listItems = [];
            for (let i = 0; i < Lists.length; i++) {
                listItems.push(
                    <Link key={i} to={`/community/${Lists[i]}`} className={styles.searchResult}>
                        {Lists[i]}
                    </Link>
                );
            }
    ...
    function generateUser(Lists) {
        if(Lists && Lists.length > 0) {
            const listItems = [];
            for (let i = 0; i < Lists.length; i++) {
                listItems.push(
                    <Link key={i} to={`/user/${Lists[i]}`} className={styles.searchResult}>
                        {Lists[i]}
                    </Link>
                );
            }
